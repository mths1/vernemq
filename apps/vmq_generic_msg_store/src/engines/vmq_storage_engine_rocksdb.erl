...
write(#state{ref=EngineRef, write_opts=WriteOpts}, WriteOps) ->
    rocksdb:write(EngineRef, WriteOps, WriteOpts).
...
init_state(DataRoot, Config) ->
    ...
    %% Parse out the open/read/write options
     {OpenOpts, _BadOpenOpts} = vmq_storage_engine_rocksdb_util:validate_options(open, FinalConfig),
     {ReadOpts, _BadReadOpts} = vmq_storage_engine_rocksdb_util:validate_options(read, FinalConfig),
     {WriteOpts, _BadWriteOpts} = vmq_storage_engine_rocksdb_util:validate_options(write, FinalConfig),
    ...
    %% todo: this is most likley wrong in rocksdb...
...
open_db(Opts, State0, RetriesLeft, _) ->
    ...
    lager:info("Try to open RocksDB database at ~p~n", [DataRoot]),
    case rocksdb:open(DataRoot, State0#state.open_opts) of
        {ok, Ref} ->
            lager:info("Opening RocksDB database at ~p~n", [DataRoot]),
        ...
        {error, {db_open, OpenErr}=Reason} ->
            case lists:prefix("Corruption: truncated record ", OpenErr) of
                true ->
                    lager:info("VerneMQ RocksDB Message Store backend repair attempt for store ~p, after error ~s. LevelDB will put unusable .log and MANIFEST filest in 'lost' folder.\n",
                            [DataRoot, OpenErr]),
                    case rocksdb:repair(DataRoot, []) of
                        ok -> % RocksDB will put unusable .log and MANIFEST files in 'lost' folder.
                ...
                false ->
                    case lists:prefix("IO error: lock ", OpenErr) of
                        true ->
                            ...
                            lager:info("VerneMQ RocksDB Message Store backend retrying ~p in ~p ms after error ~s\n",
